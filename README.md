...
| 功能 | 说明 |
| --- | --- |
| 🗺️ 收集地图 | 世界地图标出 785 个地点，密集区自动聚合成簇（簇的颜色同样反映收集进度），放大后展开；点图钉查看该地所有杯款 |
...
| 🌐 中英切换 | 顶栏 `EN / 中文` 一键切换全站语言（含城市名、国家名、地标名、排序与搜索） |
| 🔍 多维筛选 | 按城市/国家/地标搜索；按系列、类型（杯子/挂饰）、地区、国家、收集状态过滤。系列与类型标签为「点一下只看它，再点恢复全部」 |
| 👤 本地账号 | 注册/登录（数据存浏览器 localStorage），支持多账号；游客模式的标记可在登录时并入账号 |
...
151 项无头检查，覆盖筛选、中英切换、图鉴分页、账号、收藏、照片、导入导出、
...
- 账号、收藏记录与实拍照片**只存在本浏览器的 localStorage**，不会上传到任何服务器；
  换浏览器/设备或清除浏览器数据会丢失，请定期**导出备份**。
